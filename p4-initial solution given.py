...
for r in range(R):
    ...
    for k in range(1,K+1):
        ...
        z[r,k] = gm.addVar(vtype=gp.GRB.BINARY,name='z'+ str(r + 1)+ str(k))
...
for r in range(R):
    for k in range(1,K+1):
        ...
        gm.addConstr(v[k-1]-P[r]*o[r]*x[r,k]+M*B[r,k]-sum(x[r,k]*(1-P[r]) for r in range(R))>=0)
        gm.addConstr(-v[k-1]+P[r]*o[r]*x[r,k]+M*(1-B[r,k])-sum(x[r,k]*(1-P[r]) for r in range(R))>=0)
...
#for each taxi, set the request with accumulated wait time larger than 2 to be 0
for r in range(R):
    for k in range(1,K+1):
        x[r,k].start=0.0
...
tt={}
for k in range(1,K+1):
    tt[k]=min(D[u[k-1]-1,o[r]-1] for r in range(R))
    for r in range(R):
        if D[u[k-1]-1,o[r]-1]==tt[k]:
            x[r,k].start=1.0
            break
...
print("Optimal Objective Value", gm.objVal)
